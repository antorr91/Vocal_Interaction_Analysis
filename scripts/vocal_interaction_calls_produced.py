...
import pandas as pd
import numpy as np
from src.detection import onsets_offsets_detection_utils as onset_detectors
from src.detection.onsets_offsets_detection_utils import high_frequency_content, offset_detection_first_order
from src.analysis.utils_interaction_analysis import load_stimulus_log_absolute, filter_calls_produced
...
for audio_file in tqdm(audio_files, desc="Processing audio files"):
    ...
    for i, (onset, offset) in enumerate(zip(calls_produced_onsets, calls_produced_offsets)):
        ...
        # Skip calls beyond 6 bins (6 minutes)
        if time_bin > 6:
            continue
...
if all_calls_data:
    ...
    # Check for calls beyond experiment duration
    over_limit = all_calls_df[all_calls_df['Offset_Relative_Sec'] > EXPERIMENT_DURATION]
...
all_calls_df['Time_Bins_Minutes'] = (all_calls_df['Time_Bins'] - 1) * 1  # Convert to 0, 1, 2, 3, 4, 5 minutes (6 minutes total)
...
ax.set_xticklabels(['0-1 min', '1-2 min', '2-3 min', '3-4 min', '4-5 min', '5-6 min'])
...
print(f"Total figures created: 7 individual plots")